import { useEffect, useState } from "react";
  ...
  export default function FragranceInspect() {
    ...
    const [fragrance, setFragrance] = useState<FragranceDetail |
  null>(null);
    ...
    useEffect(() => {
      const fetchDetail = async () => {
        ...
        try {
          const res = await
  api.get<FragranceDetail>(`/perfumes/detail/${id}`);
      ...
      return (
        <div className="d-flex justify-content-center my-5">
          <div className="spinner-border text-primary" role="status">
            <span className="visually-hidden">Caricamento in
  corso...</span>
          </div>
        </div>
      );
    ...
    if (error || !fragrance) {
      return (
        <div className="container my-5 text-center">
          <div className="alert alert-danger">{error || "Fragranza non
  trovata."}</div>
  ...
  justify-content-center rounded-circle bg-light me-2"
    ...
    const noNotes = fragrance.top.length + fragrance.heart.length +
  fragrance.base.length === 0;

    return (
      <div className="container my-5">
        <button className="btn btn-outline-secondary mb-4" onClick={() =>
  navigate(-1)}>
          ← Indietro
        </button>
        ...
        <div className="card shadow-sm p-4">
          <div className="row g-4">
            <div className="col-md-4">
              {fragrance.imageUrl ? (
                <img src={fragrance.imageUrl} alt={fragrance.title}
  ...
  ms-2">{fragrance.releaseYear}</span>
                )}
              </p>
              {fragrance.perfumer && (
                <p className="mb-2">
              ...
              {fragrance.accords.length > 0 && (
                <div className="mb-3">
                  {fragrance.accords.map((a) => (
                    <span key={a.id} className="badge bg-info text-dark
  me-1 mb-1">
                      {a.name}
                    </span>
                  ))}
  ...
  className="text-body">{fragrance.description}</p>}
            </div>
          </div>
          ...
          <hr />
              ...
              {fragrance.description && <p className="text-body">{fragrance.description}</p>}
            </div>
              )}
              {fragrance.description && <p className="text-body">{fragrance.description}</p>}
            </div>
          ...
          {noNotes ? (
            <p className="text-muted">Nessuna nota disponibile per questa fragranza.</p>
          ) : (
        ...
        <div className="card shadow-sm p-4">
          <div className="row g-4">
            <div className="col-md-4">
              {fragrance.imageUrl ? (
                <img src={fragrance.imageUrl} alt={fragrance.title} className="img-fluid rounded" />
              ) : (
                <div
            ...
            <div className="col-md-8">
              ...
              <p className="fs-5 text-muted mb-2">
                {fragrance.brandName}
                {fragrance.releaseYear && (
                  <span className="badge bg-secondary ms-2">{fragrance.releaseYear}</span>
                )}
              </p>
              {fragrance.perfumer && (
                <p className="mb-2">
              ...
              {fragrance.accords.length > 0 && (
                <div className="mb-3">
                  {fragrance.accords.map((a) => (
                    <span key={a.id} className="badge bg-info text-dark me-1 mb-1">
                      {a.name}
                    </span>
                  ))}
                </div>
